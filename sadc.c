...
void usage(char *progname)
{
	fprintf(stderr, "%s %s [ %s ] [ <%s> [ <%s> ] ] [ <%s> ]\n",
		_("Usage:"), progname, _("options..."), _("interval"), _("count"), _("outfile"));

	fprintf(stderr, _("Options are:\n"));

	fprintf(stderr, "[ -C <%s> ] [ -d ] [ -F ] [ -I ] [ -V ]\n",
		_("comment")),
	exit(1);
}
 ...
 *
 * OUT:
 * @file_stats_size	Size of file_stats structure.
 ***************************************************************************
 */
...
void open_ofile(int *ofd, char ofile[], size_t *file_stats_size, unsigned int *flags)
{
	...
	if (ofile[0]) {
		...
		if (access(ofile, F_OK) < 0)
			...
			size = read(*ofd, &file_magic, FILE_MAGIC_SIZE);
			if (!size) {
				close(*ofd);
			...
			if ((size != FILE_MAGIC_SIZE) ||
				...
				handle_invalid_sa_file(ofd, &file_magic, ofile, size);
			}

			/* Read file standard header */
			if (read(*ofd, &file_hdr, FILE_HDR_SIZE) != FILE_HDR_SIZE) {
				/* Display error message and exit */
